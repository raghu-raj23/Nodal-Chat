...
const ChatDetailScreen = () => {
	...
	const handleSend = () => {
		if (!messageText.trim() || !currentUser || !isConnected || isSending)
			return;
		if (typingTimeoutRef.current) {
			clearTimeout(typingTimeoutRef.current);
		}
	...
	return (
		<View
			className="bg-surface-DEFAULT flex-1"
			style={{
				flex: 1,
				...
				<View className="flex-1 bg-surface-DEFAULT">
					...
					<View className="px-3 pb-3 pt-2 bg-surface border-t border-surface-light">
						<View className="flex-row items-end bg-surface-card rounded-3xl px-3 py-1.5 gap-2">
							...
							<Pressable
								className="w-8 h-8 rounded-full items-center justify-center bg-primary-DEFAULT"
								onPress={handleSend}
