...
const SOCKET_URL = "https://nodal-chat.onrender.com";
...
export const useSocketStore = create<SocketState>((set, get) => ({
	...
	connect: (token, queryClient) => {
		...
		if (existingSocket?.connected) return; // Prevent multiple connections
		...
		socket.on("socket-error", (error: { message: string }) => {
			...
			Sentry.logger.error("Socket error:", { ...error, socketId: socket.id });
		});
		...
		socket.on("new-message", (message: Message) => {
			...
			// Add message to chat's list of messages replacing the optimistic message if it exists
	...
	sendMessage: (chatId, text, currentUser) => {
		...
		Sentry.logger.info("Message sent", { chatId, text, socketId: socket.id });
